            ...
            viewLoginForm &&
                <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50 px-6">
                    <form onSubmit={handleSubmit} className="w-full max-w-[792px] bg-stone-50 h-svh flex flex-col gap-8 py-8 px-28 relative">
                        <button className="absolute right-4 top-4" onClick={() => onClose(false)}>
                        ...
                        <div>
                            ...
                            <span className="text-green-600 underline">Esqueci da senha!</span>
                        </div>
                        <div className="flex items-center justify-center">
                            <button className="w-36 h-12 bg-green-600 rounded-lg text-white font-bold" type="submit">Acessar</button>
                        </div>
                    {message && <p>{message}</p>}
                    </form>
                </div>
